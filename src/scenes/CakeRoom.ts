...
export function roseGoldMaterial(tone = '#e8b48c') {
  return new THREE.ShaderMaterial({
    vertexShader: standardVert,
    fragmentShader: /* glsl */ `
      ${fog}
      uniform vec3 uTone; uniform float uTime;
      varying vec3 vN; varying vec3 vWorldPos; varying float vDepth; varying vec2 vUv; varying vec3 vLocal;
      void main(){
    ...
    uniforms: { uTone: { value: new THREE.Color(tone) }, uTime: globalUniforms.uTime, uFogColor: globalUniforms.uFogColor, uFogDensity: globalUniforms.uFogDensity },
  });
}
