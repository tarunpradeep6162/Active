...
| + | **Someday** (inside chapter 12, Future Universe): an original manor at golden hour. It's a cream villa with arched windows and balconies, an ivy arch over the door, blossoming shrubs, fruit trees and a sunlit path. The camera walks slowly up to it. It is built entirely in code, inspired by the mood of a reference image, and copies no one's model or assets. | `src/birthday/ui/ManorScene.ts`, `manor` in the content file |
