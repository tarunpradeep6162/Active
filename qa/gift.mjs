...
await page.evaluate(() => localStorage.removeItem('bday-progress-v1'));
...
console.log(JSON.stringify({ card: ok, text: await page.evaluate(() => document.querySelector('.bd-giftcard__text')?.textContent ?? null), errors: logs.filter((l) => /error|INVALID/i.test(l)).slice(0, 5) }));
await browser.close();
