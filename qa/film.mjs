...
// count lanterns by their release events (sampling the screen can miss a wish between frames)
await page.evaluate(() => { window.__lw = new Set(); __events.on('lanternWish', (w) => __lw.add(w.index)); });
...
const released = await page.evaluate(() => __lw.size);
